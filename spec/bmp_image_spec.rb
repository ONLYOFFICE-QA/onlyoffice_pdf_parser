...
describe 'Check BmpImage class' do
  it 'Check Sub Image horizontal rectangle square pattern' do
    ...
    bmp.height = 3
    bmp.width = 5
    expect(bmp.get_sub_image(CursorPoint.new(1, 1), 2, 2).pixels).to eq([[6, 7], [11, 12]])
  end
  ...
  it 'Check Sub Image horizontal rectangle rectangle pattern' do
    ...
    bmp.height = 3
    bmp.width = 5
    expect(bmp.get_sub_image(CursorPoint.new(1, 1), 1, 2).pixels).to eq([[6], [11]])
  end
  ...
  it 'Check Sub Image vertical rectangle square pattern' do
    ...
    bmp.height = 5
    bmp.width = 3
    expect(bmp.get_sub_image(CursorPoint.new(1, 1), 2, 2).pixels).to eq([[4, 5], [7, 8]])
  end
  ...
  it 'Check Sub Image vertical rectangle rectangle pattern' do
    ...
    bmp.height = 5
    bmp.width = 3
    expect(bmp.get_sub_image(CursorPoint.new(1, 1), 1, 2).pixels).to eq([[4], [7]])
  end

  it 'Check subimage of screenshot' do
    file = BmpImage.new('spec/bmp_image_examples/SE_screenshot.bmp')
    sub_image = file.get_sub_image(CursorPoint.new(86, 934), 23, 16)
    expect(sub_image).to be_nil
  end

  it 'Find self' do
    file = BmpImage.new('spec/bmp_image_examples/SE_top_left_edge_canvas.bmp')
    pattern = 'spec/bmp_image_examples/SE_top_left_edge_canvas.bmp'
    expect(file.get_sub_image_array(pattern)).to eq([CursorPoint.new(0, 0)])
  end

  it 'Find almost all image pattern in spreadsheet' do
    file = BmpImage.new('spec/bmp_image_examples/SE_screenshot.bmp')
    pattern = 'spec/bmp_image_examples/SE_screenshot_without_top_and_left.bmp'
    expect(file.get_sub_image_array(pattern)).to eq([CursorPoint.new(1, 1)])
  end
  ...
  it 'Find font name canvas pattern in spreadsheet' do
    ...
    expect(a).to eq([CursorPoint.new(136, 37)])
  end
  ...
  it 'Find left and top canvas part pattern in spreadsheet' do
    ...
    pattern = 'spec/bmp_image_examples/SE_top_left_edge_canvas.bmp'
    a = file.get_sub_image_array(pattern)
    expect(a).to eq([CursorPoint.new(39, 111)])
  end
  ...
  it 'Find left and top canvas part pattern without show row names' do
    ...
    a = file.get_sub_image_array(pattern)
    expect(a).to eq([CursorPoint.new(38, 86)])
  end

  it 'Find spell check pattern in big screenshot' do
    file = BmpImage.new('spec/bmp_image_examples/DE_screenshot.bmp')
  ...
  it 'Find image rotary pattern in SE screenshot' do
    file = BmpImage.new('spec/bmp_image_examples/SEImageRotary.bmp')
  ...
  it 'Find image pattern_up_center pattern in pattern_up_center_full screenshot' do
    file = BmpImage.new('spec/bmp_image_examples/pattern_up_center_full.bmp')
